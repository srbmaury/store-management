...
describe('AdminJoinRequests', () => {
	...
	it('handles API.get failure gracefully when no message is received', async () => {
		API.get.mockRejectedValue({
			response: { data: '' },
		});
		...
		await waitFor(() => {
			expect(toast.error).toHaveBeenCalledWith('Failed to load join requests');
		});
	});
	...
	it('handles API.put failure gracefully when no message is received', async () => {
		API.get.mockResolvedValue({ data: mockData });
		API.put.mockRejectedValue({
			response: { data: '' },
		});
